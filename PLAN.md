...
### `ModeHLL` (approximate distinct cardinality)

- Named HLL sketch; `HLLAdd` / `HLLCount`; `Delete(name)` tombstone
- `pkg/hllx`: FNV-1a 64, `p=14`, 16384 × 6-bit dense registers (12 KiB, no header)
- Owner applies the op then fans out a **full `FlagHLL` snapshot** (hints coalesce). Inbox `FlagHLLAdd` never goes to replicas
- Missing name → `HLLCount` `ok=false`; live sketch (even estimate 0) → `ok=true`. First add creates; `Delete(name)` only drop
- `HLLAdd` is ACK-only (no Redis changed-bool). Estimates do **not** match Redis `PFCOUNT`
- PLAN §1 “HyperLogLog” means not RESP / not a Redis clone — a SuperCache mode named HLL is allowed
...
### 9.6 Structured types (Bloom / Set / ZSet / Geo / List / Hash / Counter / JSON / Bitmap / HLL)

Normative shape shared by Set, ZSet, Geo, and Hash (Bloom differs only in mutate semantics; List, Counter, JSON, Bitmap, and HLL fan out a full snapshot):
...
HLLAdd (ModeHLL; List-class snapshot, not step-3 item flag):
  1. if self != owner → ApplyPut inbox FlagHLLAdd (ACK-only)
