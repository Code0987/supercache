...
func SetLocal(h Host, name string, offset uint64, bit bool) error {
	expire := h.ExpireAt()
	cur, _ := h.Store().PeekVersion(name)
	gate := cur + 1
	applied, tooLarge := h.Store().BSet(name, offset, bit, gate, expire, h.MaxValue())
	if tooLarge {
		return ErrTooLarge
	}
	if !applied {
...
func ApplySet(h Host, name string, inbox []byte, expireAt int64) bool {
	...
	cur, _ := h.Store().PeekVersion(name)
	gate := cur + 1
	ok, tooLarge := h.Store().BSet(name, offset, bit, gate, expireAt, h.MaxValue())
	if !ok || tooLarge {
		return false
	}
	ver, _ := h.Store().PeekVersion(name)
...
func ApplyInstall(h Host, name string, blob []byte, version uint64, expireAt int64) bool {
	...
	return h.Store().BInstall(name, blob, version, expireAt)
}
...
func FetchOwner(ctx context.Context, h Host, name string) (store.Entry, bool, error) {
	...
	if err != nil || !found || !ent.IsBitmap() {
		return store.Entry{}, false, nil
	}
	if h.HoldsReplica(name) {
		_ = h.Store().BInstall(name, ent.Value, ent.Version, ent.ExpireAt)
	}
	return ent, true, nil
}
