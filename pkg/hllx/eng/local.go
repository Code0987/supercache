...
func AddLocal(h Host, name string, item []byte) error {
	...
	gate := cur + 1
	applied, tooLarge := h.Store().HLLAdd(name, item, gate, expire, h.MaxValue())
	if tooLarge {
		return ErrTooLarge
	}
	if !applied {
...
func ApplyAdd(h Host, name string, item []byte, expireAt int64) bool {
	...
	cur, _ := h.Store().PeekVersion(name)
	gate := cur + 1
	ok, tooLarge := h.Store().HLLAdd(name, item, gate, expireAt, h.MaxValue())
	if !ok || tooLarge {
		return false
	}
	ver, _ := h.Store().PeekVersion(name)
...
func ApplyInstall(h Host, name string, blob []byte, version uint64, expireAt int64) bool {
	...
	return h.Store().HLLInstall(name, blob, version, expireAt)
}
...
func FetchOwner(ctx context.Context, h Host, name string) (store.Entry, bool, error) {
	...
	if err != nil || !found || !ent.IsHLL() {
		return store.Entry{}, false, nil
	}
	if h.HoldsReplica(name) {
		_ = h.Store().HLLInstall(name, ent.Value, ent.Version, ent.ExpireAt)
	}
	return ent, true, nil
}
