...
func (m *Memory) lruVictim() *list.Element {
	for el := m.order.Back(); el != nil; el = el.Prev() {
		it := el.Value.(*lruItem)
		if (it.entry.IsTombstone() || it.entry.IsBloom() || it.entry.IsSet() || it.entry.IsZSet() || it.entry.IsGeo() || it.entry.IsList() || it.entry.IsHash() || it.entry.IsCounter() || it.entry.IsJSON() || it.entry.IsBitmap()) && !it.entry.Expired(m.now()) {
			continue
		}
		return el
	}
