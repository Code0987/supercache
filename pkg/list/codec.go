// Package list is the ModeList feature package: codec, owner write, and cluster fetch.
package list
...
func Decode(b []byte) (*List, error) {
	...
	for len(b) > 0 {
		...
		if w <= 0 {
			return nil, fmt.Errorf("list: bad uvarint")
		}
		b = b[w:]
		if uint64(len(b)) < n {
			return nil, fmt.Errorf("list: truncated item")
		}
