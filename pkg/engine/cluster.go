...
func (e *Engine) GetOrLoadLocal(ctx context.Context, keyspaceName, key string) (store.Entry, error) {
	...
	if ks.cfg.Mode == keyspace.ModeJSON {
		ent, ok := ks.store.Peek(key)
		if !ok || ent.IsTombstone() || !ent.IsJSON() {
			return store.Entry{}, ErrNotFound
		}
		return ent, nil
	}
	...
	if ks.cfg.Mode == keyspace.ModeHLL {
		ent, ok := ks.store.Peek(key)
		if !ok || ent.IsTombstone() || !ent.IsHLL() {
			return store.Entry{}, ErrNotFound
		}
		return ent, nil
	}
	if ks.cfg.Mode == keyspace.ModeTopK {
		ent, ok := ks.store.Peek(key)
		if !ok || ent.IsTombstone() || !ent.IsTopK() {
			return store.Entry{}, ErrNotFound
		}
		return ent, nil
	}
	if ks.cfg.Mode == keyspace.ModeCMS {
		ent, ok := ks.store.Peek(key)
		if !ok || ent.IsTombstone() || !ent.IsCMS() {
			return store.Entry{}, ErrNotFound
		}
		return ent, nil
	}
	if ks.cfg.Mode == keyspace.ModeVectorSet {
		ent, ok := ks.store.Peek(key)
		if !ok || ent.IsTombstone() || !ent.IsVectorSet() {
			return store.Entry{}, ErrNotFound
		}
		return ent, nil
	}
	if ks.cfg.Mode == keyspace.ModeStream {
		ent, ok := ks.store.Peek(key)
		if !ok || ent.IsTombstone() || !ent.IsStream() {
			return store.Entry{}, ErrNotFound
		}
		return ent, nil
	}
